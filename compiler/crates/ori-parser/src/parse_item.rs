...
use ori_ast::item::{
    AbiLabel, AliasDecl, EnumDecl, EnumVariant, ExternBlock, ExternMember,
    FuncDecl, FuncSignature, ImplementDecl, ImportDecl, Item, ItemWithAttrs,
    NamedField, NamespaceDecl, Param, ParamKind, SourceFile, StructDecl,
    StructField, TopConst, TopVar, TraitDecl, TraitMember,
};
...
impl<'src> Parser<'src> {
    ...
    pub fn parse_source_file(&mut self) -> SourceFile {
        ...
        while self.at(&TokenKind::Import) {
            if let Some(i) = self.parse_import() { imports.push(i); }
        }
        ...
        while !self.at_eof() {
            if let Some(item) = self.parse_item_with_attrs() {
                ...
                self.synchronize(&[
                    TokenKind::Func, TokenKind::Public, TokenKind::Struct,
                    TokenKind::Enum, TokenKind::Trait, TokenKind::Implement,
                    TokenKind::Alias, TokenKind::Const, TokenKind::Var,
                    TokenKind::Extern, TokenKind::At,
                ]);
            }
        }
        let end = self.tokens.last().map(|t| t.span).unwrap_or(start);
        SourceFile { namespace, imports, items, span: start.cover(end) }
    }

    fn parse_namespace(&mut self) -> Option<NamespaceDecl> {
        let start = self.expect(&TokenKind::Namespace)?;
        let name = self.parse_qualified_name()?;
        Some(NamespaceDecl { span: start.cover(name.span), name })
    }

    fn parse_import(&mut self) -> Option<ImportDecl> {
        let start = self.advance().unwrap().span; // import
        let path = self.parse_qualified_name()?;
        let alias = if self.eat(&TokenKind::As) { Some(self.parse_name()?) } else { None };
        let end = alias.as_ref().map(|a| a.span).unwrap_or(path.span);
        Some(ImportDecl { path, alias, span: start.cover(end) })
    }

    fn parse_item_with_attrs(&mut self) -> Option<ItemWithAttrs> {
        let mut attrs = Vec::new();
        while self.at(&TokenKind::At) {
            if let Some(attr) = self.parse_attr() { attrs.push(attr); }
        }
    ...
    fn parse_attr(&mut self) -> Option<Attr> {
        ...
        let end = args.last().map(|_| self.current_span()).unwrap_or(name.span);
        Some(Attr { name, args, span: start.cover(end) })
    }

    fn parse_visibility(&mut self) -> Visibility {
        if self.eat(&TokenKind::Public) { Visibility::Public } else { Visibility::Private }
    }
    ...
    pub fn parse_func_decl(&mut self, vis: Visibility) -> Option<FuncDecl> {
        ...
        Some(FuncDecl { visibility: vis, is_mut, name, type_params, params, return_ty, where_clause, body, span: start.cover(end) })
    }
    ...
    fn parse_func_signature(&mut self, vis: Visibility) -> Option<FuncSignature> {
        ...
        Some(FuncSignature { visibility: vis, is_mut, name, type_params, params, return_ty, where_clause, span: start.cover(end) })
    }
    ...
    fn parse_param(&mut self) -> Option<Param> {
        ...
        if self.at(&TokenKind::SelfKw) {
            ...
            return Some(Param { name, ty: ori_ast::ty::Type::Named(ori_ast::common::QualifiedName::single(
                ori_ast::common::Name::new("Self", tok.span)
            )), kind: ParamKind::Required, span: tok.span });
        }
        let name = self.parse_name()?;
        self.expect(&TokenKind::Colon)?;
        ...
        if self.eat(&TokenKind::DotDot) {
            ...
            return Some(Param { name, ty, kind: ParamKind::Variadic, span });
        }
    ...
    fn parse_struct_decl(&mut self, vis: Visibility) -> Option<StructDecl> {
        ...
        let end = self.expect(&TokenKind::End)?;
        Some(StructDecl { visibility: vis, name, type_params, where_clause, fields, methods, span: start.cover(end) })
    }
    ...
    fn parse_struct_field(&mut self) -> Option<StructField> {
        ...
        let span = start.cover(self.current_span());
        Some(StructField { name, ty, contract, span })
    }
    ...
    fn parse_enum_decl(&mut self, vis: Visibility) -> Option<EnumDecl> {
        ...
        let end = self.expect(&TokenKind::End)?;
        Some(EnumDecl { visibility: vis, name, type_params, variants, span: start.cover(end) })
    }
    ...
    fn parse_enum_variant(&mut self) -> Option<EnumVariant> {
        ...
        let fields = if self.at(&TokenKind::LParen) {
            ...
            while !self.at(&TokenKind::RParen) && !self.at_eof() {
                ...
                fs.push(NamedField { name: n, ty, span: sp });
                if !self.eat(&TokenKind::Comma) { break; }
            }
            self.expect(&TokenKind::RParen)?;
            fs
        ...
        Some(EnumVariant { name, fields, span: start.cover(end) })
    }
    ...
    fn parse_trait_decl(&mut self, vis: Visibility) -> Option<TraitDecl> {
        ...
        while !self.at(&TokenKind::End) && !self.at_eof() {
            ...
            let has_body = !self.at_any(&[TokenKind::Func, TokenKind::Public,
                                          TokenKind::Mut, TokenKind::End]);
            if has_body && !is_mut {
                let body = self.parse_block()?;
                let end = self.expect(&TokenKind::End)?;
                let decl = FuncDecl {
                    visibility: sig.visibility, is_mut: sig.is_mut,
                    name: sig.name.clone(), type_params: sig.type_params.clone(),
                    params: sig.params.clone(), return_ty: sig.return_ty.clone(),
                    where_clause: sig.where_clause.clone(), body,
        ...
        let end = self.expect(&TokenKind::End)?;
        Some(TraitDecl { visibility: vis, name, type_params, where_clause, members, span: start.cover(end) })
    }
    ...
    fn parse_implement_decl(&mut self) -> Option<ImplementDecl> {
        ...
        let end = self.expect(&TokenKind::End)?;
        Some(ImplementDecl { type_params, trait_name, for_type, where_clause, methods, span: start.cover(end) })
    }
    ...
    fn parse_alias_decl(&mut self, vis: Visibility) -> Option<AliasDecl> {
        ...
        Some(AliasDecl { visibility: vis, name, type_params, ty, span })
    }
    ...
    fn parse_top_const(&mut self, vis: Visibility) -> Option<TopConst> {
        ...
        Some(TopConst { visibility: vis, name, ty, value: Box::new(value), span })
    }
    ...
    fn parse_top_var(&mut self, vis: Visibility) -> Option<TopVar> {
        ...
        let value = self.parse_expr()?;
        let span = start.cover(value.span());
        Some(TopVar { visibility: vis, name, ty, value: Box::new(value), span })
    }
    ...
    fn parse_extern_block(&mut self) -> Option<ExternBlock> {
        ...
        let abi = if self.at(&TokenKind::Ident) {
            ...
            match self.slice(tok.span) {
                "c" | "C" => { self.advance(); AbiLabel::C }
                "host"    => { self.advance(); AbiLabel::Host }
                _         => AbiLabel::C,
            }
        } else { AbiLabel::C };
        let mut members = Vec::new();
        ...
        let end = self.expect(&TokenKind::End)?;
        Some(ExternBlock { abi, members, span: start.cover(end) })
    }
    ...
    fn parse_extern_member(&mut self, vis: Visibility) -> Option<ExternMember> {
        ...
        if self.at(&TokenKind::Func) {
            ...
            let return_ty = if self.eat(&TokenKind::Arrow) { Some(self.parse_type()?) } else { None };
            let end = return_ty.as_ref().map(|t| t.span()).unwrap_or(name.span);
            Some(ExternMember::Func { visibility: vis, name, params, return_ty, span: start.cover(end) })
            ...
            let ty = self.parse_type()?;
            let span = start.cover(ty.span());
            Some(ExternMember::Var { visibility: vis, name, ty, span })
        } else {
            let span = self.current_span();
            self.error("parse.expected_extern_member", "expected `func` or `var` in extern block", span);
            None
        }
    }
}

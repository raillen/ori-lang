...
fn stdlib_func_sig(path: &str) -> Option<(Vec<Ty>, Ty)> {
    if let Some(sig) = crate::stdlib::stdlib_func_sig(path) {
        return Some(sig);
    }
    let path = crate::stdlib::canonical_stdlib_path(path).unwrap_or(path);
    let sig = match path {
        ...
        "ori.string.to_bytes" | "string.to_bytes" => (vec![Ty::String], Ty::Bytes),
        "ori.string.from_bytes" | "string.from_bytes" => (
            vec![Ty::Bytes],
            Ty::Result(Box::new(Ty::String), Box::new(Ty::String)),
        ),
        "ori.bytes.len" | "bytes.len" => (vec![Ty::Bytes], Ty::Int),
        "ori.bytes.concat" | "bytes.concat" => (vec![Ty::Bytes, Ty::Bytes], Ty::Bytes),
        "ori.bytes.slice" | "bytes.slice" => (vec![Ty::Bytes, Ty::Int, Ty::Int], Ty::Bytes),
        "ori.bytes.to_hex" | "bytes.to_hex" => (vec![Ty::Bytes], Ty::String),
        "ori.bytes.from_hex" | "bytes.from_hex" => (
            vec![Ty::String],
            Ty::Result(Box::new(Ty::Bytes), Box::new(Ty::String)),
        ),
        "ori.bytes.decode_utf8" | "bytes.decode_utf8" => (
            vec![Ty::Bytes],
            Ty::Result(Box::new(Ty::String), Box::new(Ty::String)),
        ),
        "ori.bytes.get" | "bytes.get" => (vec![Ty::Bytes, Ty::Int], Ty::U8),
        ...
        | "ori.fs.create_dir"
        | "fs.create_dir"
        | "ori.files.create_dir"
        | "files.create_dir" => (vec![Ty::String], Ty::Bool),
        ...
        "ori.fs.copy" | "fs.copy" | "ori.files.copy" | "files.copy" | "ori.fs.rename"
        | "fs.rename" | "ori.files.rename" | "files.rename" => {
            (vec![Ty::String, Ty::String], Ty::Bool)
        }
        _ => return None,
    };

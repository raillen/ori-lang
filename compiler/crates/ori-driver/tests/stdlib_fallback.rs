...
fn test_find_stdlib_root_resolves_via_cwd_fallback() {
    ...
    assert!(root.is_some(), "Should find stdlib using cwd fallback upward search");
    let root_path = root.unwrap();
    assert!(root_path.join("string").is_dir(), "stdlib root should contain 'string' directory");
}

...
fn render_one(cache: &SourceCache, diag: &Diagnostic, color: bool) {
    ...
    eprintln!(
        "{}{}{}[{}]{}: {}{}{}",
        c(color, BOLD), c(color, sev_color), sev_label,
        diag.code,
        c(color, RESET), c(color, BOLD), diag.message, c(color, RESET)
    );
    ...
    for label in &diag.labels {
        if let Some(file) = cache.get(label.file_id) {
            ...
            eprintln!(
                "  {}{}-->{} {}:{}:{}",
                c(color, DIM), c(color, CYAN), c(color, RESET),
                file.path.display(), line, col
            );
            eprintln!("   {}{}|{}", c(color, DIM), gutter, c(color, RESET));
            eprintln!(
                "{}{}{}{}|{} {}",
                c(color, DIM), line_num, c(color, RESET), c(color, DIM), c(color, RESET), line_text
            );

            // Underline
            let col0  = (col as usize).saturating_sub(1);
            let len   = (label.span.len()).max(1).min(line_text.len().saturating_sub(col0));
            let under = "^".repeat(len);
            eprintln!(
                "   {}{}|{} {}{}{}{}{}",
                c(color, DIM), gutter, c(color, RESET),
                " ".repeat(col0), c(color, sev_color), c(color, BOLD), under, c(color, RESET)
            );
            ...
            if !label.message.is_empty() {
                eprintln!(
                    "   {}{}|{} {}{} {}{}",
                    c(color, DIM), gutter, c(color, RESET),
                    " ".repeat(col0), c(color, DIM), label.message, c(color, RESET)
                );
            }
            eprintln!("   {}{}|{}", c(color, DIM), gutter, c(color, RESET));
        }
    ...
    if let Some(action) = &diag.action {
        eprintln!("   {}= action:{} {}", c(color, DIM), c(color, RESET), action);
    }
...
pub fn print_summary(errors: usize, warnings: usize, color: bool) {
    if errors == 0 && warnings == 0 {
        ...
        if errors > 0 {
            eprint!("{}{}{}{} error(s){}", c(color, BOLD), c(color, RED), errors, c(color, RESET), c(color, RESET));
        }
        if errors > 0 && warnings > 0 { eprint!(", "); }
        if warnings > 0 {
            eprint!("{}{}{}{} warning(s){}", c(color, BOLD), c(color, YELLOW), warnings, c(color, RESET), c(color, RESET));
        }
        eprintln!();
    }
}

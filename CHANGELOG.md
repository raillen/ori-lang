...
- **Roadmap (Rust removal):** Decisão arquitetural fechada — remoção da dependência de Rust seguirá híbrido A→B→D: Phase 1 (completa, `[Unreleased]`) bundle `rust-lld` + CRT discovery próprio para Windows MSVC, Linux GNU e macOS; Phase 2 (completa, `[Unreleased]`) system linker via `ORI_USE_SYSTEM_LINKER=1` (`link.exe`/`ld`/`ld64` direto com CRT discovery, sem `rust-lld` nem `rustc`); Phase 3 (longo prazo, opcional) JIT Cranelift para `ori run`/`test` (elimina link step). `ORI_NATIVE_LINKER` permanece como escape hatch raw sem CRT discovery (diagnóstico), distinto de `ORI_USE_SYSTEM_LINKER`.
...
- **Stdlib Phase 0 (prelude loading + expansão Layer 2):** Infraestrutura de prelude loading para `stdlib/*.orl` entregue (ver `### Adicionado`). Boundary Layer 1/2/3 confirmado na prática: `stdlib/string/utils.orl` (Layer 2) importa `ori.string` (Layer 1 manifesto + runtime FFI) via mecanismo de import normal — sem `extern "C"` declarations no `.orl`, sem merge manifesto/DefMap. Convenção de path fechada: `ori.X.Y.Z` → `stdlib/X/Y/Z.orl` (dots → separador de diretório, extensão `.orl`). Funções stdlib em `.orl` devem ser `public` para serem callable cross-namespace (mesma regra de visibilidade de funções de usuário). Expansão Layer 2 entregue: `center`, `count`, `equals_ignore_case`, `default` portados em `.orl` sobre primitivas Layer 1 (`pad_left`/`pad_right`/`slice`/`to_lower`/`len`). Naming collision documentada: `len` como nome de variável local colide com símbolo interno `ori_len` do runtime nativo — convenção adotada: usar prefixo qualificador (`s_len`, `sub_len`, etc.) em código `.orl` stdlib. Próximos marcos (futuro): port gradual de mais funções Layer 2 (`reverse`, `capitalize`, `title`, `swap_case` sobre `str.chars` + list ops), port Layer 3 algoritmos (`ori.tree` traversal em `.orl` sobre primitives Layer 1), formalização em spec cap. 15.

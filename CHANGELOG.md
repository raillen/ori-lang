...
- **Roadmap (Rust removal):** Decisão arquitetural fechada — remoção da dependência de Rust seguirá híbrido A→B→D: Phase 1 (completa, `[Unreleased]`) bundle `rust-lld` + CRT discovery próprio para Windows MSVC, Linux GNU e macOS; Phase 2 (futuro) system linker fallback (`link.exe`/`ld` direto via `ORI_NATIVE_LINKER`); Phase 3 (longo prazo, opcional) JIT Cranelift para `ori run`/`test` (elimina link step).

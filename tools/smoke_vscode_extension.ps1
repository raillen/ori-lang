...
try {
    if (-not $SkipCargoBuild) {
        Invoke-Checked { cargo build -p ori-driver -p ori-lsp } "cargo build -p ori-driver -p ori-lsp"
    }
    ...
    if (-not $SkipLspE2e) {
        Invoke-Checked { cargo test -p ori-lsp --test e2e } "cargo test -p ori-lsp --test e2e"
    }

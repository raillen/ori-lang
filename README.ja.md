...
Ori AOT / CPython / Rust release のマイクロベンチ要約は英語・ポルトガル語の
ガイドにあります（この README 日本語版はリンクのみ）:
...
概要: Ori はこれらのカーネルで CPython より約 **4–46×** 高速、リスト churn では
Rust の約 **1.5×** 遅れ、密な整数ループでは Rust より大きく遅れます（codegen の
余地）。詳細と注意事項は上記ガイドを参照。

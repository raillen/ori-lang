...
| **LANG-MEM-1** | Audit dtor × edges overlap + scenario tests S1–S4 | 1 | M | partial | **Analysis done 2026-07-17 — bug CONFIRMED**: dtor + edge both release each managed field → shared child freed while binding alive (UAF repro S4d) + owned-temp leak on field assign (S4b, masked). Fix = ADR single cascade owner (rec: edges) + 3-point patch. Note: [`historico/nim-study-2026-07-17-c1.md`](historico/nim-study-2026-07-17-c1.md) |

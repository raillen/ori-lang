...
| **LANG-MEM-2** | Edge completeness matrix for all managed types | 1 | M | partial | **2026-07-17**: matrix measured on real binary (note [`historico/nim-study-2026-07-17-c2.md`](historico/nim-study-2026-07-17-c2.md)). **Fixed**: print/f-string temp leaks (7 allocs per 5-part f-string). **Clean**: optional/result/tuple/cycles. **Confirmed gap (next)**: map/set owned-arg leaks paired with borrowed `get` treated as owned — fix both sides together or UAF. Closures sync capture = pre-existing backend residual |
